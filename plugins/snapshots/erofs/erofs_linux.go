...
type SnapshotterConfig struct {
	...
	enableVerity bool
	// verityHashAlgorithm specifies the hash algorithm for dm-verity (default: sha256)
	verityHashAlgorithm string
...
// WithVerity enables dm-verity for committed snapshots
func WithVerity() Opt {
	return func(config *SnapshotterConfig) {
		config.enableVerity = true
	}
}

// WithVerityHashAlgorithm sets the hash algorithm for dm-verity
func WithVerityHashAlgorithm(algo string) Opt {
	return func(config *SnapshotterConfig) {
		config.verityHashAlgorithm = algo
	}
}

// WithVerityCombined enables combined mode for dm-verity
func WithVerityCombined() Opt {
	return func(config *SnapshotterConfig) {
		config.verityCombined = true
	}
}
...
func (s *snapshotter) mounts(snap storage.Snapshot, info snapshots.Info) ([]mount.Mount, error) {
	...
	options = append(options, s.config.ovlOptions...)
...
func (s *snapshotter) Commit(ctx context.Context, name, key string, opts ...snapshots.Opt) error {
	...
	err := s.ms.WithTransaction(ctx, false, func(ctx context.Context) error {
		...
		if _, err := os.Stat(layerBlob); err != nil {
			...
			// Configure verity if enabled
			verity := erofsutils.VerityConfig{
				Enabled:  s.config.enableVerity,
				HashAlgo: s.config.verityHashAlgorithm,
				Mode:     erofsutils.VerityModeSeparate,
			}
			if s.config.verityCombined {
				verity.Mode = erofsutils.VerityModeCombined
			}

			err = erofsutils.ConvertErofsWithVerity(ctx, layerBlob, upperDir, verity, nil)
			if err != nil {
				return err
			}
