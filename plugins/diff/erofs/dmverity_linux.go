...
func (s *erofsDiff) formatDmverityLayer(ctx context.Context, layerBlobPath string) error {
	...
	// Configure dm-verity parameters for hash size calculation
	opts.HashOffset = hashOffset
	opts.DataBlocks = uint64(dataBlocks)

	// Calculate the actual hash tree size needed
	...
	// Calculate total size needed: data + optional superblock + hash tree
