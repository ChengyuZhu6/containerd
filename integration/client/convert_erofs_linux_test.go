...
func TestConvertErofsRaw(t *testing.T) {
	...
	// UpdateManifestPlatform must preserve arch/os/variant and only augment OSFeatures.
	defPlat := platforms.DefaultSpec()
	assert.Equal(t, defPlat.Architecture, manifestDesc.Platform.Architecture, "Architecture must be preserved")
	assert.Equal(t, defPlat.OS, manifestDesc.Platform.OS, "OS must be preserved")
	assert.Equal(t, defPlat.Variant, manifestDesc.Platform.Variant, "Variant must be preserved")
...
func TestConvertErofsUpdateManifestIdempotent(t *testing.T) {
	if testing.Short() {
		t.Skip()
	}
	skipIfNoMkfsErofs(t)
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			firstRef := testImage + "-erofs-idem1-" + tc.name
			defer deleteImage(t, ctx, client, firstRef)
			first := convertToErofs(t, ctx, client, testImage, firstRef, tc.compression)

			secondRef := testImage + "-erofs-idem2-" + tc.name
			defer deleteImage(t, ctx, client, secondRef)
			second := convertToErofs(t, ctx, client, firstRef, secondRef, tc.compression)

			cs := client.ContentStore()
			for name, img := range map[string]*images.Image{"first": first, "second": second} {
				manifestDesc, mani := resolveManifest(t, ctx, cs, img.Target)
				require.NotNil(t, manifestDesc.Platform, name)
				n := 0
				for _, f := range manifestDesc.Platform.OSFeatures {
					if f == "erofs" {
						n++
					}
				}
				assert.Equalf(t, 1, n, "%s pass: OSFeatures contains %d 'erofs' entries, want 1 (features=%v)",
					name, n, manifestDesc.Platform.OSFeatures)
				...
				assert.Equalf(t, 1, erofsCount, "%s pass: config os.features contains %d 'erofs' entries (features=%v)",
					name, erofsCount, feats)
			}

			_, first1 := resolveManifest(t, ctx, cs, first.Target)
			_, second1 := resolveManifest(t, ctx, cs, second.Target)
			require.Equal(t, len(first1.Layers), len(second1.Layers))
			for i := range first1.Layers {
				assert.Equalf(t, first1.Layers[i].Digest, second1.Layers[i].Digest,
					"layer[%d] digest should be stable across idempotent conversion", i)
			}
		})
	}
}
