...
func addAnnotations(ocispec specs.Spec, config *vc.SandboxConfig, runtime RuntimeConfig) error {
	...
	// Pass io.kubernetes.cri.sandbox-memory annotation to sandbox config
	if value, ok := ocispec.Annotations["io.kubernetes.cri.sandbox-memory"]; ok {
		config.Annotations["io.kubernetes.cri.sandbox-memory"] = value
	}
